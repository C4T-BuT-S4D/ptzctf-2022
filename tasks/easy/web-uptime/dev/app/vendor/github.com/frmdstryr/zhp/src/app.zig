...
pub const ServerRequest = struct {
    ...
    pub fn init(allocator: Allocator, app: *Application, connection: *ServerConnection) !ServerRequest {
    ...
    fn requestLoop(self: *ServerConnection, conn: net.StreamServer.Connection) !void {
        ...
        var server_request: *ServerRequest = undefined;
        {
            ...
            if (app.request_pool.get()) |c| {
                ...
                server_request.* = try ServerRequest.init(app.allocator, app, self);
        ...
        while (true) {
            ...
            // Request handler might've already sent the response.
            if (!(self.io.closed or response.finished)) {
            ...
            if (self.io.closed or !keep_alive) {
                std.log.info("returning, keepalive: {}", .{keep_alive});
                return;
            }
        }
    }
...
pub const Route = struct {
    ...
    // /pages/<int>/edit
    pub fn create(comptime path: []const u8, comptime T: type) Route {
        if (path[0] != '/') {
            @compileError("Route url path must start with /");
        }
        return Route{ .pattern = path, .handler = createHandler(T) };
    }

    pub fn static(comptime path: []const u8, comptime file_path: []const u8) Route {
    ...
    pub fn websocket(comptime path: []const u8, comptime T: type) Route {
        return create(path, handlers.WebsocketHandler(T));
    }
};
...
const default_middleware = [_]Middleware{
    Middleware.create(web.middleware.RedirectMiddleware),
};
...
pub const Application = struct {
    ...
    // Server Config
    // ------------------------------------------------------------------------
    pub const routes: []const Route = root.routes[0..];

    pub const middleware: []const Middleware = if (@hasDecl(root, "middleware"))
        root.middleware[0..] ++ default_middleware[0..]
    else
    ...
    running: std.atomic.Atomic(bool) = std.atomic.Atomic(bool){ .value = false },
    options: Options,
    ...
    pub fn start(self: *Application) !void {
        ...
        self.running.store(true, std.atomic.Ordering.SeqCst);
        ...
        defer {
            self.running.store(false, std.atomic.Ordering.SeqCst);
        }

        while (self.running.load(std.atomic.Ordering.SeqCst)) {
            // Grab a frame
            ...
            if (comptime std.io.is_async) {
                ...
                server_conn.startRequestLoop(conn);
            }
        }
    }
    ...
    pub fn processResponse(self: *Application, server_request: *ServerRequest) !void {
        // Zig is absolute shit
        if (middleware.len > 0) {
            if (middleware[0].processResponse) |f| {
                try f(self, server_request);
            }
        }
        if (middleware.len > 1) {
            if (middleware[1].processResponse) |f| {
                try f(self, server_request);
            }
        }
        if (middleware.len > 2) {
            if (middleware[2].processResponse) |f| {
                try f(self, server_request);
            }
        }
    }
